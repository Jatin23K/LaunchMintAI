        ...
        <div ref={ref} className="p-12 bg-white text-slate-900 min-h-screen" style={{ width: '800px', position: 'fixed', top: 0, left: 0, opacity: 0, pointerEvents: 'none', zIndex: -9999 }}>
            ...
            <section className="mb-16">
                <div className="flex items-center gap-2 mb-6 border-l-8 border-emerald-500 pl-4">
                    ...
                    <span className="ml-auto text-sm font-black text-emerald-600 border-2 border-emerald-600 px-3 py-1 rounded">RISK: {data?.god_mode?.risk_score ?? 'N/A'}</span>
                </div>
                <div className="p-8 bg-slate-50 border-2 border-slate-200 rounded-3xl">
                    <p className="text-2xl font-bold text-slate-900 italic mb-6 leading-tight">"{data?.god_mode?.macro_verdict ?? 'Analysis pending'}"</p>
            ...
            <section className="mb-16">
                ...
                <div className="grid grid-cols-2 gap-8 mb-8">
                    <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Forecast TAM ({market?.forecast_year ?? "2030"})</div>
                        <div className="text-4xl font-black text-slate-900">{market?.forecast_tam ?? market?.size ?? 'N/A'}</div>
                    </div>
                    <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Current TAM ({market?.current_year ?? "2024/25"})</div>
                        <div className="text-4xl font-black text-slate-900">{market?.current_tam ?? 'N/A'}</div>
                    </div>
                    <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Veracity Index</div>
                        <div className="text-2xl font-bold text-emerald-600">{data?.forensics ? `${Math.round((data.forensics.veracity_index ?? 0) * 100)}%` : "Verified"}</div>
                    </div>
                    <div className="p-6 bg-slate-50 border border-slate-200 rounded-2xl">
                        <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">Forensic Weight</div>
                        <div className="text-2xl font-bold text-blue-600">{data?.forensics ? `${Math.round((data.forensics.confidence_score ?? 0) * 100)}%` : (market?.confidence ?? 'N/A')}</div>
                    </div>
                </div>
                <div className="p-6 bg-slate-50 border-2 border-slate-200 rounded-2xl">
                    <h4 className="font-black text-slate-900 mb-2 uppercase text-xs tracking-widest flex items-center gap-2">
                        ...
                        Market Timing: {market?.timing?.label ?? 'N/A'}
                    </h4>
                    <p className="text-slate-600 italic text-sm leading-relaxed">{market?.timing?.rationale ?? ''}</p>
                    <div className="mt-4 text-[10px] text-slate-400 font-bold uppercase tracking-tighter">Source Link: {market?.source_url ?? 'N/A'}</div>
                </div>
            </section>
                    ...
                    {competitors.slice(0, 3).map((c, i) => (
                        <div key={i} className="border-4 border-slate-900 rounded-[2rem] overflow-hidden">
                            <div className="bg-slate-900 text-white p-6 flex justify-between items-center">
                                <h3 className="text-2xl font-black uppercase italic tracking-tighter">{c?.name ?? 'Unknown'}</h3>
                            ...
                            <div className="p-8 grid grid-cols-3 gap-8 border-b-2 border-slate-100">
                                <div className="space-y-4">
                                    ...
                                    <div className="text-xs space-y-2">
                                        <p><span className="font-bold">Funding:</span> {c?.market_fin?.funding ?? 'N/A'}</p>
                                ...
                                <div className="space-y-4">
                                    ...
                                    <div className="text-xs space-y-2">
                                        <p><span className="font-bold">Pricing:</span> {c?.product_intel?.pricing ?? 'N/A'}</p>
                                        <p><span className="font-bold">UX:</span> {c?.product_intel?.ux_friction ?? "N/A"}</p>
                                        <p><span className="font-bold">Stack:</span> {c?.technical_infra?.stack ?? 'N/A'}</p>
                                    </div>
                                </div>
                                <div className="space-y-4">
                                    ...
                                    <div className="text-xs space-y-2">
                                        ...
                                        <p><span className="font-bold text-red-600 uppercase">Fatal Weakness:</span> {c?.weakness ?? 'N/A'}</p>
                                    </div>
                                </div>
                            </div>
                            <div className="p-8 bg-slate-50 italic">
                                ...
                                <p className="text-slate-900 font-bold leading-relaxed">"{c?.kill_strategy ?? 'N/A'}"</p>
                            </div>
                        </div>
                    ))}
                </div>
