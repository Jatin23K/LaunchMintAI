...
function ValidatorApp({ onSave, data, setData, setStatus }: { onSave: (report: RealData) => void, data: RealData | null, setData: (r: RealData | null) => void, setStatus: (s: 'idle' | 'processing' | 'active') => void }) {
    ...
    const runAnalysis = async (text: string = input) => {
        ...
        try {
            ...
            // Run existing analysis and DS insights in parallel
            const [response, dsResponse] = await Promise.allSettled([
                axios.post(`${apiBase}/analyze`, { idea: text }),
                axios.post(`${apiBase}/ds_insights`, {
                    idea: text,
                    market_data: {},
                    competitors: []
                })
            ...
            if (response.status === 'fulfilled') {
                ...
                setData({ ...response.value.data, idea: text });
                setError(null);

                // If DS insights also succeeded, update competitors for sentiment
                if (dsResponse.status === 'fulfilled') {
                    const dsResult = dsResponse.value.data?.data;
                    setDsData(dsResult || null);
                }
                setStatus('active');
            } else {
