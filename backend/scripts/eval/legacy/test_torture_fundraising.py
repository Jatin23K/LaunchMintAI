...
EXTENSION_ID = "fundraising-intelligence"
        ...
        if response.status_code == expected_status:
            try:
                ...
                if data.get("status") == "success":
                    slides = len(data.get("data", {}).get("deck_outline", []))
                    print_status(test_id, name, "PASS", f"({slides} Slides Generated in {elapsed:.2f}s)")
...
# --- THE FUNDRAISING TORTURE SUITE ---
print(f"{Colors.HEADER}💸 STARTING FUNDRAISING VALIDATION (EXTENSION 11){Colors.ENDC}")
print("-" * 80)

run_test(1, "Standard Pre-Seed", {"idea": "Uber for Dog Walking", "stage": "Pre-Seed", "amount": "$500k"})
run_test(2, "Series A Ask", {"idea": "AI Cybersecurity", "stage": "Series A", "amount": "$5M"})
run_test(3, "Raw String Input", "A drone delivery network for pizza")
run_test(4, "Hardware Context", {"idea": "Nuclear Fusion Reactor"})
run_test(5, "Niche & Abstract", "Telepathy app for twins")
run_test(6, "Minimal Input", "Lemonade stand")
run_test(7, "Long Context", "Context: " + ("financial history " * 100))
...
run_test(12, "Numeric Input", 1000000)
run_test(13, "List Input", ["Investor 1", "Investor 2"])
